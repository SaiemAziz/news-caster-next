...
const register = () => {
  ...
  let handleCheckUserName = async (e) => {
    ...
    if (e.target.value === "") return setAvailable(null);
    ...
    let data = await res.json();
    let allUsers = data.data;
    let fetchedUserNames = allUsers.map((user) =>
  ...
  let handlerForm = async (e) => {
    ...
    registerUser(email, password)
      ...
      .catch((err) => {
        ...
        setLoad(false);
        toast?.error(
          err.code.replace("auth/", "").replaceAll("-", " ").toUpperCase()
        );
      });
  };
        ...
        >
          <div className="relative w-full max-w-xs font-semibold">
            <input
              ...
              type="email"
              id="floating_email"
              className="block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent border-l-4 border-[#097ef6] outline-0 focus:ring-0 focus:border-[#097ef6] peer"
              placeholder=" "
              name="email"
            />
            <label
              htmlFor="floating_email"
              className="absolute  text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0]  px-2 peer-focus:px-2 peer-focus:text-[#097ef6]  peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1"
            >
              Email
            </label>
          </div>
          ...
          {available === "true" && (
            <p className="text-center font-bold text-sm text-success">
              User name available
            </p>
          )}
          <div className="relative w-full max-w-xs font-semibold">
          ...
          "
            ...
            <label
              ...
              className="absolute  text-[#097ef6] duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0]  px-2 peer-focus:px-2 peer-focus:text-[#097ef6]  peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 left-1"
            >
              Image
            </label>
          </div>
          ...
          <div className="flex gap-3 ml-auto">
            <p className="text-[#097ef6] font-semibold text-xs">
              Show Password
            </p>
            <input
              type="checkbox"
          ...
          {load ? (
            ...
            <input
              type="submit"
              className="btn border-0 btn-info text-center text-white rounded-full w-full py-2 mt-10 bg-[#097ef6]"
              value="Register"
            />
          )}
          <Link
            href="/user/login"
            className="btn border-0 btn-link text-info text-center w-full mt-0"
          >
            Already a Member?
          </Link>
        </form>
